...
public class RigidEnemy : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		if (this.state == RigidEnemy.EnemyState.active || this.state == RigidEnemy.EnemyState.tumbling || this.state == RigidEnemy.EnemyState.recovering || this.state == RigidEnemy.EnemyState.falling)
		{
			...
			if (!Physics.Raycast(this.root.position, Vector3.down, ref raycastHit, this.ik.heightAboveGround * 3f, this.ik.whatIsGround))
			{
		...
		else
		{
			...
			float num3 = Mathf.Clamp(1f - this.RootHeight() / this.ik.heightAboveGround, -1f, 1f);
	...
	public void Concuss()
	{
		...
		base.Invoke("GetUp", this.recoverTime * Random.Range(0.7f, 1.5f));
	}
	...
	private void ConfigureLegs(bool makeRagdoll)
	{
		...
		for (int i = 0; i < this.ik.legs.Length; i++)
		{
			...
			while (j > 0)
			{
				...
				if (makeRagdoll)
				{
					transform.gameObject.AddComponent<HingeJoint>().connectedBody = transform.parent.GetComponent<Rigidbody>();
				}
				else
				{
			...
			{
				rigidbody.isKinematic = !makeRagdoll;
				rigidbody.interpolation = (makeRagdoll ? 1 : 0);
			}
	...
	{
		RaycastHit raycastHit;
		if (Physics.Raycast(this.root.position, Vector3.down, ref raycastHit, 10f, this.ik.whatIsGround))
		{
	...
	{
		float y = this.root.transform.eulerAngles.y;
